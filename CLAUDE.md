...
- **Always verify.** After a change, run `npm run test`, `npm run lint`, and `npm run build` before considering it done. For anything touching the 3D scene or UI, also boot the dev server and actually look at it (see "Verifying visually") — **and always additionally verify on the Android emulator** (see "Mobile (Capacitor)"), not desktop-only. The emulator runs software rendering (no GPU passthrough here), which has already caught real bugs desktop Chromium didn't (e.g. the Outline postprocessing corruption bug) — treat the on-device pass as the default step for any visual change, not an optional extra reserved for "big" ones.
...
**Wings (horizontal floor growth, M4):** `slotX(slotIndex)` is a pure function of `slotIndex` alone (not of a floor's total slot count), so a floor's slots `0..3` never move when it gains "wing" slots `4, 5, ...` beyond the starting `ROOMS_PER_FLOOR` (`store.ts`'s `expandFloor` action just increments that floor's `slotCount`) — new slots simply extend further along `+X`. Because of this, a floor's slab has a **fixed left edge** (slot 0's edge) and a right edge that grows with `slotCount`, so `floorSlabPosition`/`floorSlabSize` take an explicit `slotCount` param and are *not* centered at `x=0` once a floor has been expanded — `Building.tsx`'s ground plane and `Floor.tsx`'s corridor railing both have to use the same computed center-X, not assume 0. `MAX_WING_EXPANSIONS_PER_FLOOR` (`roomTypes.ts`) caps this so a floor can't grow unboundedly wide.
...
**Gotcha: a long-running emulator instance can silently degrade its GPU translation layer and render a totally blank white canvas** (DOM/HUD overlay still works fine, no JS console errors, no crash — `adb logcat` shows nothing more alarming than benign `GFXSTREAM: set vao to self, no-op` transport warnings). Confirmed via a controlled test: an old, previously-verified-good commit reproduced the exact same blank canvas on the same emulator instance, ruling out a code regression. Fix: kill and reboot the emulator (`adb emu kill`, then relaunch per the loop below and `adb wait-for-device` + poll `sys.boot_completed`) — do this *before* spending time bisecting app code for a rendering bug that only shows up after the emulator's been up a long time (this one had been running ~24h+ continuously across sessions). A fresh boot also lands on a lock screen — send `adb shell input keyevent 82` (or swipe) before screenshotting, or you'll capture a black screen and misdiagnose that too.

On-device testing also showed the default camera (`App.tsx`'s `camera={{position, fov}}`, `Scene.tsx`'s `OrbitControls target`) left excessive empty sky above the building on a tall phone aspect ratio — barely noticeable on a squat desktop viewport but wasteful on a real portrait screen. Retuned by iterating screenshot-to-screenshot directly on the emulator (closer position, narrower FOV, target shifted to include the elevator core rather than just the room row) rather than by exact FOV/aspect trigonometry — verify any further camera changes on-device, not just via desktop Playwright, since the two aspect ratios read very differently.
